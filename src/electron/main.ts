...
const connectionReverseEngineerChannel =
  "connections:reverse-engineer-request";
const connectionForwardEngineerChannel =
  "connections:forward-engineer-request";
...
function projectError(action: "open" | "save"): Error {
  ...
  return new Error(
    `[${code}] Could not ${action} the drawDB project. Check the file and permissions, then try again.`,
  );
}
...
function resolveRendererEntry(): string {
  ...
  if (!rendererDirectory) {
    throw new Error(
      "drawDB desktop renderer build was not found. Run npm run build:desktop before launching Electron.",
    );
  }
