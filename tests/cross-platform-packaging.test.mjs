...
const windowsNsisArtifactPath = path.join(
  repositoryRoot,
  "dist-installers",
  "drawDB Desktop-0.1.0-win-x64-setup.exe",
);
const windowsPortableArtifactPath = path.join(
  repositoryRoot,
  "dist-installers",
  "drawDB Desktop-0.1.0-win-x64-portable.exe",
);
...
function assertExecutableArtifact(filePath, label) {
  ...
  assert.ok(stats.size > 1024 * 1024, `${label} must be a real packaging artifact`);
  assert.equal(
...
describe("SS-012 cross-platform packaging configuration", () => {
  ...
  it("has real Windows x64 NSIS and portable packaging outputs", () => {
    assertExecutableArtifact(windowsNsisArtifactPath, "Windows x64 NSIS installer");
